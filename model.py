...
def main():
    ...
    q_steps_ahead_pred(q=10,dd=1,lag=3,plot=True, df=pre_pandemic, test_prop=0.2)

    # Prediction using daily seasonality
    t_full=pd.array(pd.DatetimeIndex(pre_pandemic.iloc[:,1]).astype(np.int64))/1000000000
    dt = t_full[1]-t_full[0]
    print(f"Data sampling interval is {dt} seconds")

    d=math.floor(24*60*60/dt)
    q_steps_ahead_pred(q=d,dd=d, lag=3, plot=True, df=pre_pandemic, test_prop=0.2)

    # Prediction using weekly seasonality
    d = math.floor(7*24*60*80/dt)
    q_steps_ahead_pred(q=d,dd=d, lag=3, plot=True, df=pre_pandemic, test_prop=0.2)

    # Prediction using monthly seasonality
    d= math.floor(4*7*24*60*60/dt)
    q_steps_ahead_pred(q=d, dd=d, lag=3, plot=True, df=pre_pandemic, test_prop=0.2)
